...
def main():
    ...
    for line in f:
        ...
        # for final model
        tweets.append((tweet_text, tweet_sentiment))
    ...
    # now build final model for test on external dataset
    test_tweets = []
    try:
        test_file = sys.argv[2]
    except IndexError:
        print("No test file, done")
        exit()
    
    # re-initialize 2D dictionary to store confusion matrix
    classes = {"POS","NEG","NEU"}
    result_svm = {}
    for row in classes:
        result_svm[row] = result_svm.get(row,{})
    ...
    """Load test file"""
    f = open(sys.argv[2],encoding="utf8") # pass data file as first parameter
    for line in f:
        try: 
            tweet = line.split("\t")
        except:
        ...
        tweet_sentiment = tweet[2].rstrip() # sentiments are last item on list, so they get \n added
        test_tweets.append((tweet_text, tweet_sentiment))
